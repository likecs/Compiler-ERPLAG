...
pNode createNonTerminal(pNode Parent, non_term non_term_1, tag t, int no_of_child, int level)
{
	pNode temp = createParseTreeNode(Parent, t, level);
	temp->e.nt = non_term_1;
...
parseTree parseInputSourceCode(char * testCaseFile, hashTable h, Grammar G, int print)
{
	...
	int error_lexer = populateLexemeTable(testCaseFile, h, 0);
	if (error_lexer == 1)
	{
		parse_status = 0;
	...
	while((currentToken!=NULL) && (((stack.top->t == terminal) && (stack.top->ge.t != $)) || (stack.top->t == non_terminal)))
	{
		...
		{ 
			...
			{ 
				...
				else
				{
					...
					{
						printf("\nERROR: The token (%s) for lexeme (%d) does not match at line no. %d. The expected token here is %s", local_token,currentToken->val.i,currentToken->lno,stackToken);
					}
					else if (currentToken->s == RNUM)
					{
						printf("\nERROR: The token (%s) for lexeme (%f) does not match at line no. %d. The expected token here is %s", local_token,currentToken->val.f,currentToken->lno,stackToken);
					}
					else if (currentToken->s == ID)
					{
						printf("\nERROR: The token (%s) for lexeme (%s) does not match at line no. %d. The expected token here is %s", local_token,currentToken->val.s,currentToken->lno,stackToken);
					}
					else 
					{
						local_lexeme = getLexeme(currentToken->s);
						printf("\nERROR: The token (%s) for lexeme (%s) does not match at line no. %d. The expected token here is %s", local_token,local_lexeme,currentToken->lno,stackToken);
					}
					printf("\n%d\n",stack.top->ge.t);
					flag = 1;
		...
		{ 
			...
			{ 
				...
				if (currentToken->s == NUM)
				{
					printf("\nERROR: The token (%s) for lexeme (%d) does not match at line no. %d. The expected token here are %s", local_token,currentToken->val.i,currentToken->lno,stackToken);
				}
				else if (currentToken->s == RNUM)
				{
					printf("\nERROR: The token (%s) for lexeme (%f) does not match at line no. %d. The expected token here are %s", local_token,currentToken->val.f,currentToken->lno,stackToken);
				}
				else if (currentToken->s == ID) 
				{
					printf("\nERROR: The token (%s) for lexeme (%s) does not match at line no. %d. The expected token here are %s", local_token,currentToken->val.s,currentToken->lno,stackToken);
				}
				else 
				{
					local_lexeme = getLexeme(currentToken->s);
...
void printParseTree(char * testCaseFile, hashTable h, Grammar G)
{
	...
	if (parse_status == 0)
	{
		printf("There are errors in parsing. So no parsetree is created\n");
		return ;
	}
	printf("%-15s:%-5s:%-12s:%-10s:%-25s:%-12s:%-20s","lexCurrentNode", "lNo", "token", "valIfNo", "pNode", "IsLeaf(y/n)", "nodeSymbol");
	printParseTreeQueue(Tree.root);
}